...
- Extend `web/e2e/ui_e2e.mjs` at a 390px viewport to assert the mobile key bar, static terminal toolbar, collapsed bars, and viewport-bounded server switcher. Keep the key-sequence mapping in one directly testable function.
