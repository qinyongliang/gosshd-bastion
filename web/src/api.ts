...
export const api = {
  ...
  batchCommandHistories: (params: Record<string, unknown>) => request<{ histories: BatchCommandHistory[]; total: number; page: number; page_size: number }>(`/api/batch-command-histories?${queryString(params)}`),
  recordBatchCommandHistory: (body: Record<string, unknown>) => request<{ history: BatchCommandHistory }>("/api/batch-command-histories", post(body)),
